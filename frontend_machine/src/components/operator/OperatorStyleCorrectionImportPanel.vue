...
import { downloadOperatorStyleCorrectionTemplate } from "../../utils/operatorStyleCorrectionImportExcel";

const props = defineProps({
  isAdmin: { type: Boolean, default: false },
  selectedDate: { type: String, default: "" },
  templateRows: { type: Array, default: () => [] },
  locationFilter: { type: String, default: "ALL" },
  importing: { type: Boolean, default: false },
  importInputKey: { type: Number, default: 0 },
  importFileName: { type: String, default: "" },
  importPreviewRows: { type: Array, default: () => [] },
  importPreviewDisplayRows: { type: Array, default: () => [] },
  importDuplicateRows: { type: Array, default: () => [] },
  importErrorRows: { type: Array, default: () => [] },
  importStats: {
    type: Object,
    default: () => ({
      totalExcelRows: 0,
  ...
  importErrorMessage: { type: String, default: "" },
  importSuccessMessage: { type: String, default: "" },
...
function handleDownloadTemplate() {
  ...
  try {
    const result = downloadOperatorStyleCorrectionTemplate(props.templateRows, {
    ...
    templateMessage.value =
      err?.message || "Gagal mengunduh template Koreksi Style.";
  }
}
</script>
...
<template>
  <section v-if="isAdmin" class="import-panel" :class="{ embedded }">
    <div class="import-toolbar">
      <h3>Koreksi Style Sesi</h3>
      <p class="mode-hint">
        Isi <strong>Style Baru</strong> (wajib) &amp; Proses Baru (opsional).
        Harus ada di Master IE.
      </p>
        ...
        <table>
          ...
          <tbody>
            <tr
              ...
              :key="`${row.workDate}-${row.uuid}-${row.excelRowNumber}`"
            >
              <td>{{ row.workDate }}</td>
